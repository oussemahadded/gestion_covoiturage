...
class Message
{
    ...
    /** Envoyer un message */
    public function send(int $expediteurId, int $destinataireId, string $contenu, ?int $trajetId = null): int|false
    {
        $stmt = $this->pdo->prepare(
            'INSERT INTO messages (expediteur_id, destinataire_id, trajet_id, contenu)
             VALUES (?, ?, ?, ?)'
        );
        $ok = $stmt->execute([$expediteurId, $destinataireId, $trajetId, $contenu]);
